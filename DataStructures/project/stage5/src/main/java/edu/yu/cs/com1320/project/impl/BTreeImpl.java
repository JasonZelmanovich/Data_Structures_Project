...
public class BTreeImpl<Key extends Comparable<Key>, Value> implements BTree<Key, Value> {
    ...
    public Value get(Key key) {
        ...
        if(entry != null)
        {
            if(entry.val instanceof Runnable){
                ((Runnable) entry.val).run();
                try {
                    pm.delete(key);
                } catch (IOException e) {
                    throw new RuntimeException(e);
                }
                entry = this.get(this.root, key, this.height);
            }
    ...
    public Value put(Key key, Value val) {
        ...
        {
            Value previous_entry = alreadyThere.val;
            if(previous_entry instanceof Runnable) {
                ((Runnable) previous_entry).run();
                try {
                    pm.delete(key);
                } catch (IOException e) {
                    throw new RuntimeException(e);
                }
                previous_entry = this.get(key);
            }
    ...
    public void moveToDisk(Key k) throws Exception {
        ...
        Runnable reference = () -> {
            Value doc = null;
            try {
                doc = pm.deserialize(k);
            }catch(IOException e){
                e.printStackTrace();
            }
            Entry e = this.get(this.root,k,height);
            e.val = doc;
        };
        this.put(k,(Value) reference);
    }
